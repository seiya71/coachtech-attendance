                    ...
                    @foreach ($attendance->breaks as $index => $break)
                        <div class="d-flex mb-2">
                            <input type="time" name="breaks[{{ $index }}][start]"
                                value="{{ old("breaks.$index.start", optional($break->start)->format('H:i')) }}" class="form-control me-2">
                            <input type="time" name="breaks[{{ $index }}][end]"
                                value="{{ old("breaks.$index.end", optional($break->end)->format('H:i')) }}" class="form-control me-2">
                        </div>
                    @endforeach
                    <div class="d-flex">
