...
@section('content')
    <div class="container">
        ...
        <div class="flex items-center justify-between mb-4">
            <form method="GET" action="{{ route('attendance.index', ['month' => $prevMonth]) }}">
            ...
            <form method="GET" action="{{ route('attendance.index', ['month' => $nextMonth]) }}">
            ...
            <tbody>
                @foreach ($attendanceData as $day)
                    <tr>
                        ...
                        <td class="border px-2 py-1">{{ $day['break_time'] ?? '-' }}</td>
                        <td class="border px-2 py-1">{{ $day['work_time'] ?? '-' }}</td>
